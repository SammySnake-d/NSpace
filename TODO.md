...
- ✅（v0.11.2）**I-26 可选列（创建/添加日期）点列头无法排序**（用户报告）：根因 SortSpec.Key 枚举缺 created/added，列注册诚实标了 sortable=false。修复：枚举补全+DirectoryReader 比较器补 case（nil 归 distantPast）+两列开排序+默认排序设置项同步扩展；单测（严格弱序+nil）与 UI 真效果断言（sortDescriptors 链路→model.sort 真变）双锁。
