...
- ⬜ **I-33 多选→单选切换选中响应卡顿**（用户报告）：疑状态栏 selectedBytes 汇总/斑马重绘在大选中集变化时同步阻塞。定位+修复+流畅度断言。
...
- ⬜ **M27 冲突体验增强**（用户点名对齐 QSpace）：同目录复制必询冲突（现疑走静默两者保留）；冲突面板加"重命名"选项；批量冲突像 QSpace 左下角按钮组一次选定应用全部。
